...
| `/logo-preview/`                                 | Keep page live but exclude from XML sitemap.            | Utility/dev preview page appeared as `sitemap_only` and is not a public SEO target.                  | Verify after deploy: `/logo-preview/` should not appear in `sitemap-0.xml`.                               |
