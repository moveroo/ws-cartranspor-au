...
function validateKey(key) {
  if (!/^[A-Za-z0-9_-]{8,128}$/.test(key)) {
    throw new Error('IndexNow key must be 8-128 characters and contain only letters, numbers, underscores, or dashes.');
  }
  return key;
}
...
async function sitemapUrls(siteUrl) {
  const candidates = ['sitemap-index.xml', 'sitemap.xml', 'sitemaps.xml', 'sitemap_index.xml', 'sitemap-0.xml'];
...
async function submit({ host, key, keyLocation, urls, dryRun }) {
  ...
  for (const [index, urlList] of batches.entries()) {
    ...
    if (dryRun) {
      results.push({ batch: index + 1, urlCount: urlList.length, httpStatus: null, status: 'dry_run' });
      continue;
    }
    ...
    results.push({
      ...
      status: response.status === 200 ? 'submitted' : response.status === 202 ? 'accepted_pending_key_validation' : 'failed',
    });
  }
...
async function main() {
  ...
  const isProductionDeploy = process.env.VERCEL_ENV === 'production' || (process.env.NETLIFY === 'true' && process.env.CONTEXT === 'production');
  ...
  const keyLocation = process.env.INDEXNOW_KEY_LOCATION || indexNow.keyFileUrl || (siteUrl + '/' + key + '.txt');
