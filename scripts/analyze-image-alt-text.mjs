...
async function main() {
  ...
  console.log(`📋 Found ${allFiles.length} files (${pageFiles.length} pages, ${componentFiles.length} components)\n`);
  ...
  console.log(`  ✅ Good Alt Text: ${goodAlt.length} (${Math.round((goodAlt.length / (allImages.length || 1)) * 100)}%)`);
  console.log(`  ⚠️  Generic Alt Text: ${genericAlt.length} (${Math.round((genericAlt.length / (allImages.length || 1)) * 100)}%)`);
  console.log(`  ❌ Missing Alt Text: ${missingAlt.length} (${Math.round((missingAlt.length / (allImages.length || 1)) * 100)}%)\n`);
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
        ...
        missingAlt: missingAlt.map(img => ({ file: img.file, src: img.src, suggestion: img.suggestion })),
        genericAlt: genericAlt.map(img => ({ file: img.file, src: img.src, current: img.altText, suggestion: img.suggestion })),
    }, null, 2)
  );
